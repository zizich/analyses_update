...
async def process_start(message: Message, state: FSMContext):
    ...
    for i, (name, female, patronymic, birth_date, phone, email, address, city) in enumerate(user, start=1):
        if all([name, female, patronymic, birth_date, phone, email, city, address]):
    ...
    cursor_db.execute("""INSERT OR IGNORE INTO users (user_id) VALUES (?)""",
                      (user_id,))
    conn.commit()
    ...
    for i, (name, female, patronymic, birth_date, phone, email, address, city) in enumerate(user, start=1):
        if city is None:
            ...
            await message.answer(text="\u203C\uFE0F<b>Введите имя:</b> ")
        elif female is None:
            await message.answer(text="Для доступа к сервисам медицинских услуг, "
                                      "пожалуйста, пройдите регистрацию!"
                                      "\n\U0001F3E0<b>Профиль:</b>"
