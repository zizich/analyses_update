...
async def process_add_address_done(message: Message, state: FSMContext):
    ...
    for i, (city, sampling, exit_db, address, phone, bank, all_sale,
            nurse_delivery, nurse_income_day) in enumerate(database_db.fetchall(), start=1):
...
async def add_all_info(call: CallbackQuery, state: FSMContext):
    ...
    database_db.execute(
        ...
        (unique_user, fio, birth_date, phone, email, city, address, None, None, None, None, user_id))
...
async def process_delete_people_one(call: CallbackQuery):
    ...
    fio = database_db.execute(f"""SELECT fio FROM users WHERE user_id = ?""", (unique_num,)).fetchone()[0]

    database_db.execute(f"""DELETE FROM users WHERE user_id = ? """, (unique_num,))
    connect_database.commit()
...
async def process_edit_people(call: CallbackQuery):
    ...
    database_db.execute(f"""SELECT * FROM users WHERE user_id = ?""", (unique_us_id,))
    db_profile = database_db.fetchall()

    for i, (user_id, fio, birth_date, phone, email, city, address, subscribe, info, others, photo,
            reference) in enumerate(db_profile, start=1):
...
async def process_city_people_one(call: CallbackQuery):
    ...
    async def kb_add_city_other():
        database_db.execute("""SELECT * FROM cities_payment""")
        for i, (city, sampling, exit_db, address, phone, bank, all_sale, nurse_delivery,
...
async def process_add_city_people_one(call: CallbackQuery):
    ...
    database_db.execute(f"""UPDATE users SET city = ? WHERE user_id = ?""",
                        (city, id_user))
    connect_database.commit()
